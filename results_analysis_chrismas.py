...
#%%
# load data and model
odtrips = np.load('results/christmas_demand_normalized_v2.npy')
...
def plot_best_fits(c, model, cluster_params, all_losses):
    ...
    # fig.savefig(f"figures/fit_christmas_cluster{c+1}.pdf", bbox_inches='tight')
...
# fig.savefig(f"figures/resilience_patterns_christmas.pdf", bbox_inches="tight")
# %%
# load data and model
odtrips = np.load('results/christmas_demand_normalized_2018.npy')
odtrips = odtrips[~np.isnan(odtrips).any(axis=1)]
model = DTWClustering(odtrips, 3)
model.clusters, model.centroids, cluster_params, all_losses = pickle.load(open('results/results_snowstorm_v2.pkl', 'rb'))
